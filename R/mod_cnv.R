      ...
      column(width = 12,
             box(title = "Inputs", width = 12, collapsible = TRUE, collapsed = FALSE, status = "info", solidHeader = TRUE,
                 ...
                 div(style = "position:relative;",
                     ...
                     # Qploidy2 info banner (spans full width above both sides)
                     fluidRow(
                       column(width = 5,
                              div(style = "background:#eef6fb; border-left:4px solid #17a2b8; border-radius:4px; padding:10px 14px; margin-bottom:10px; font-size:13px;",
                                  HTML(paste0(
                                    "<b>Files generated by Qploidy2</b> &mdash; use ",
                            ...
                            fluidRow(
                              column(6,
                                     fileInput(ns("qploidy_standarize_result"),
                                               label = HTML("Upload Qploidy2 standardize results (from function `write_qploidy_standardization`)"),
                                               accept = c(".tsv.gz"), 
                                               width = "70%")
                              ),
                              column(width = 6,
...
mod_cnv_server <- function(input, output, session, parent_session){
  ...
  observeEvent(input$cnv_load_dataset, {
    ...
    req(!is.null(paths))
    updateProgressBar(session, "pb_cnv", value = 10, title = "Loading Qploidy2 HMM results...")
    ...
    updateProgressBar(session, "pb_cnv", value = 50, title = "Qploidy2 HMM results loaded.")
  ...
  standardized <- reactive({
    ...
    updateProgressBar(session, "updated_hmm_bar", value = 10, title = "Reading Qploidy2 standardization file...")
  ...
  cnv_summary_info <- function() {
    ...
    cat(
      ...
      paste("Qploidy2:", packageVersion("Qploidy2")), "\n",
