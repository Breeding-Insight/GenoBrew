...
app_server <- function(input, output, session) {
  ...
  callModule(mod_mk_select_server,
             "mk_select_1",
             parent_session = session)
  ...
  #Check for updates from GitHub for GenoBrew
  ...
  observeEvent(input$updates_info_button, {
    ...
    package_name <- "GenoBrew"
    repo_name <- "GenoBrew" # GitHub repo name
