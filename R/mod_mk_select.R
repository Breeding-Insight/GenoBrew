        ...
        box(
          ...
          div(
            ...
            fluidRow(
              column(
                width = 6,
                numericInput(ns("dist_ploidy"),
                  label = "Ploidy",
                  value = NA, min = 1L, max = 12L, step = 1L, width = "30%"
                )
              ),
              column(width = 1),
              column(
            ...
            fluidRow(
              column(
                width = 6,
                actionButton(ns("mk_select_start"), "Load VCF",
                  icon = icon("upload"),
                  class = "btn-info"
                )
              ),
          ...
          fluidRow(
            column(
              ...
              conditionalPanel(
                ...
                div(
                  style = "border-left: 4px solid #6f42c1; background:#f8f9fa; border-radius:4px; padding: 12px 12px 12px 16px; margin-bottom:8px;",
                  HTML(paste0(
...
mod_mk_select_server <- function(input, output, session, parent_session) {
  ...
  observeEvent(input$load_dataset, {
    ...
    req(
      file.exists(paths$vcf_path) | grepl("^https?://", paths$vcf_path),
  ...
  output$ui_colour_by <- renderUI({
    choices <- c(
      "None" = "", "Missing" = "missing", "Depth" = "depth",
  ...
  output$marker_distribution_plot <- renderPlot(
    {
      ...
      p <- plot_marker_positions(
        marker_stats = {
          if (!is.null(mk_select_items$marker_stats_filtered)) mk_select_items$marker_stats_filtered else if (!is.null(mk_select_items$common_marker_stats)) mk_select_items$common_marker_stats else mk_select_items$marker_stats
        },
        colour_by = colour_by,
  ...
  observeEvent(input$apply_filters, {
    ...
    filtered <- stats_filter(
      vcf = if (input$selected_dataset == "Common Markers") mk_select_items$vcf_common else mk_select_items$vcf_input,
  ...
  observeEvent(input$goPar, {
    ...
    # select specific tab
    updateTabsetPanel(
      session = parent_session, inputId = "Select_markers_tabset",
